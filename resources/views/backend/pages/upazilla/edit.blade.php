        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6"><h2><i class="fa fa-table"> {{ __('backend/upazilla.edit_upazilla') }}</i></h2>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.upazilla.update', $upazilla->id) }}" method="post">
                            @csrf
                            <div class="form-row">
                                <div class="col-md-6 form-group">
                                    <label for="name" class="col-form-label">Upazilla <span class="text-danger">*</span></label>
                                    <input type="text" name="name" id="name" class="form-control" value="{{ $upazilla->name }}" required>
                                </div>
                                <div class="col-md-6 form-group">
                            ...
                            <div class="form-row">
                                <div class="col-md-4 form-group">
                                    ...
                                    <select name="country" id="country" class="form-control" required>
                                        ...
                                        <option value="{{ $country }}" {{ $country == $upazilla->country ? 'selected' : '' }}>{{ $country }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4 form-group">
                                    <label for="shipping_cost" class="col-form-label">Shipping Cost <span class="text-danger">*</span></label>
                                    <input type="number" name="shipping_cost" id="shipping_cost" value="{{ $upazilla->shipping_cost }}" class="form-control" required>
                                </div>
                                <div class="col-md-4 form-group">
                                    <label class="col-form-label" for="status">{{ __('backend/default.status') }} <span class="text-danger">*</span></label>
                                    <div>
                                        <select class="form-control" name="status" id="status" required>
                                            <option value='1' {{ $upazilla->status == 1 ? 'selected' : '' }} >Active</option>
                                            <option value='0' {{ $upazilla->status == 0 ? 'selected' : '' }} >Deactive</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
