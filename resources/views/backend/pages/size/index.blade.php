...
<div class="app-title">
  <div>
    <h1><i class="fa fa-arrows"></i> {{ __('backend/size.size') }} {{ __('backend/default.management') }}</h1>
  </div>
  <ul class="app-breadcrumb breadcrumb">
    <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i><a href="{{ route('admin.home') }}">{{ __('backend/default.dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('backend/size.size') }}</li>
  </ul>
</div>
<div class="row">
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <div class="row">
          <div class="col-md-6">
            <h2><i class="fa fa-table"></i>&nbsp;{{ __('backend/size.size') }}</h2>
          </div>
      ...
      <div class="card-body">
        ...
        <div class="toggle-table-column">
          ...
          <a href="#" class="toggle-vis" data-column="1"><b>{{ __('backend/size.size') }}</small></b></a> |
        ...
        <div class="table-responsive pt-1">
          <table id="datatable" class="table table-bordered table-hover display">
            ...
            <tbody>
              ...
              <tr class="{{ $size->status == 0 ? 'deactive_':'' }}">
                <td>{{ $loop->index + 1 }}</td>
                <td>
                  <span>
                    <span class="fa fa-arrows" style="margin: 3px 5px 5px 0"></span>
                ...
                <td class="action">
                  <div class="btn-group">
                    ...
                    @if($key == 0)
                    <button data-toggle="modal" data-target="#edit_page{{ $size->id }}" class="btn btn-info text-white"><i class="fa fa-edit"></i></button>
                    @else
                    <button class="btn text-white {{ $size->status == 0? ' btn-secondary disabled':' btn-danger' }}" onClick="deleteMethod({{ $size->id }})" role="button" {{ $size->status == 0? 'disabled':'' }}><i class="fa fa-minus-circle"></i></button>
                    @endif
                    @endif
                    @endforeach
                  </div>
                  {{-- edit Modal --}}
                  <div class="modal fade" id="edit_page{{ $size->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-pencil-square"></i> {{ __('backend/size.size') }} {{ __('backend/default.update') }}</h5>
                        ...
                        <div class="modal-body">
                          <form action="{{ route('admin.size.update', $size->id) }}" method="POST">
                            ...
                            <div class="form-group">
                              ...
                              <div>
                                <input type="text" class="form-control" name="title" value="{{ $size->title }}" id="title" required>
                              </div>
                            </div>
                            <div class="form-group">
  ...
  <div class="modal fade" id="add_new" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel"><i class="fa fa-plus-square"></i> {{ __('backend/size.add_size') }}</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
