                    ...
                    <div class="btn-group">
                          @foreach($permissions->submenus as $key => $permission)
                            @if(\App\Models\Menu::checkBodyMenu($permission->id, $bodyMenu->in_body))
                                ...
                                <a onclick="prmt_deleteMethod(`{{route('admin.brand.permanently_delete', $row->id)}}`)" class="btn btn-danger ml-1" title="Permanently delete your data..!"><i class="fa fa-trash"></i></a>
                                <!--<button class="btn btn-danger ml-1" onClick="prmt_deleteMethod({{ json_encode($row->id) }})" role="button"><i class="fa fa-trash"></i></button>-->
